                  ...
                  <div key={flow.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                    <div className="flex items-center gap-4 px-5 py-4">
                      <button onClick={() => toggleActive(flow)} className="flex-shrink-0">
                        {flow.is_active ? <ToggleRight className="w-8 h-8 text-[#6366f1]" /> : <ToggleLeft className="w-8 h-8 text-gray-300" />}
                      </button>
                      ...
                      <div className="flex items-center gap-1 flex-shrink-0">
                        <button onClick={() => openEdit(flow)} className="p-2 text-gray-400 hover:text-[#6366f1] hover:bg-[#6366f1]/5 rounded-lg transition-colors"><Pencil className="w-4 h-4" /></button>
                ...
                <div key={w.id} className="bg-white rounded-2xl border border-gray-100 p-5">
                  <div className="flex items-start gap-4">
                    <button onClick={() => toggleWebhook(w)} className="flex-shrink-0 mt-0.5">
                      {w.is_active ? <ToggleRight className="w-8 h-8 text-[#6366f1]" /> : <ToggleLeft className="w-8 h-8 text-gray-300" />}
                    </button>
                    <div className="flex-1 min-w-0 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1">
                          <button onClick={() => openEditWebhook(w)} className="p-2 text-gray-400 hover:text-[#6366f1] hover:bg-[#6366f1]/5 rounded-lg transition-colors"><Pencil className="w-4 h-4" /></button>
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl border border-gray-100 max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
              <div>
                ...
                <input type="text" value={formName} onChange={e => setFormName(e.target.value)} placeholder="Ex: Follow-up Sem Contato" className="w-full px-3 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none focus:border-[#6366f1] focus:bg-white transition-all" />
              </div>
              <div>
                ...
                <select value={formStage} onChange={e => setFormStage(e.target.value)} className="w-full px-3 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-sm text-gray-700 focus:outline-none focus:border-[#6366f1] focus:bg-white transition-all cursor-pointer">
                  ...
                  {formSteps.map((step, i) => (
                    <div key={i} className="bg-gray-50 rounded-xl p-4 border border-gray-100 space-y-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <input type="number" min={1} value={step.delay_minutes} onChange={e => updateStep(i, 'delay_minutes', parseInt(e.target.value) || 1)} className="w-16 px-2 py-1 bg-white border border-gray-200 rounded-lg text-[13px] text-center text-gray-800 focus:outline-none focus:border-[#6366f1] transition-all" />
                        <select value={step.delay_unit} onChange={e => updateStep(i, 'delay_unit', e.target.value)} className="px-2 py-1 bg-white border border-gray-200 rounded-lg text-[13px] text-gray-700 focus:outline-none focus:border-[#6366f1] transition-all cursor-pointer">
                          <option value="minutes">Minutos</option>
                          <option value="hours">Horas</option>
                          <option value="days">Dias</option>
                        </select>
                      </div>
                      <textarea value={step.message} onChange={e => updateStep(i, 'message', e.target.value)} placeholder={`Oi {nome}, tudo bem? 👋`} rows={3} className="w-full px-3 py-2.5 bg-white border border-gray-200 rounded-xl text-[13px] text-gray-800 placeholder:text-gray-400 focus:outline-none focus:border-[#6366f1] transition-all resize-none" />
                      <p className="text-[11px] text-gray-400">Use <code className="bg-gray-200 px-1 rounded">{'{nome}'}</code> para o nome do lead</p>
                    </div>
                  ))}
                </div>
                <button onClick={addStep} className="mt-3 w-full py-2.5 border border-dashed border-gray-200 rounded-xl text-[13px] text-gray-400 hover:border-[#6366f1] hover:text-[#6366f1] hover:bg-[#6366f1]/5 transition-all flex items-center justify-center gap-2">
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex gap-3">
              ...
              <button onClick={handleSaveFlow} disabled={saving} className="flex-1 py-2.5 bg-[#6366f1] text-white rounded-xl text-[13px] font-medium hover:bg-[#4f46e5] active:scale-[0.98] transition-all disabled:opacity-40 flex items-center justify-center gap-2">
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl border border-gray-100" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-6 py-5 space-y-5">
              <div>
                ...
                <input type="text" value={wName} onChange={e => setWName(e.target.value)} placeholder="Ex: Formulário Site Principal" className="w-full px-3 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none focus:border-[#6366f1] focus:bg-white transition-all" />
              </div>
              {!editWebhook && (
                <div>
                  <label className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider block mb-1.5">Canal WhatsApp</label>
                  <select value={wChannelId} onChange={e => setWChannelId(Number(e.target.value))} className="w-full px-3 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-sm text-gray-700 focus:outline-none focus:border-[#6366f1] focus:bg-white transition-all cursor-pointer">
              ...
              <div>
                ...
                <textarea value={wMessage} onChange={e => setWMessage(e.target.value)} placeholder={`Oi {nome}, vi que você se interessou! 👋`} rows={4} className="w-full px-3 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-[13px] text-gray-800 placeholder:text-gray-400 focus:outline-none focus:border-[#6366f1] focus:bg-white transition-all resize-none" />
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex gap-3">
              ...
              <button onClick={handleSaveWebhook} disabled={savingWebhook} className="flex-1 py-2.5 bg-[#6366f1] text-white rounded-xl text-[13px] font-medium hover:bg-[#4f46e5] active:scale-[0.98] transition-all disabled:opacity-40 flex items-center justify-center gap-2">
