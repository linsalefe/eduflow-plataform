    ...
    <AppShell>
      <div className="px-4 lg:px-6 max-w-[1400px] mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
          ...
          <button onClick={() => openNewSchedule()} className="flex items-center gap-2 px-4 py-2.5 bg-primary text-white rounded-xl hover:bg-primary/90 transition-colors text-sm font-medium w-fit">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4 mb-6">
          {[
            { label: 'Hoje', value: stats.today, color: 'text-primary', bg: 'bg-primary/10' },
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          <button onClick={() => setView('calendar')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${view === 'calendar' ? 'bg-primary text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
            <Calendar className="w-4 h-4 inline mr-1.5" />Calendário
          </button>
          <button onClick={() => setView('list')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${view === 'list' ? 'bg-primary text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
              ...
              {selectedDate ? (
                <>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={() => openNewSchedule(selectedDate)} className="p-1.5 bg-primary/10 text-primary rounded-lg hover:bg-primary/20">
              ...
              <div className="divide-y divide-gray-100">
                {filteredSchedules.map(s => (
                  <div key={s.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-gray-50 transition-colors">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${s.type === 'voice_ai' ? 'bg-primary/10' : 'bg-pink-50'}`}>
                      {s.type === 'voice_ai' ? <Bot className="w-5 h-5 text-primary" /> : <UserCheck className="w-5 h-5 text-pink-500" />}
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setShowModal(false)}>
          <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              {!editingSchedule && (
                <>
                  <div>
                    ...
                    <input value={formName} onChange={e => setFormName(e.target.value)} className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary" placeholder="Nome completo" />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-gray-500 mb-1 block">Telefone *</label>
                    <input value={formPhone} onChange={e => setFormPhone(e.target.value)} className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary" placeholder="5581999999999" />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-gray-500 mb-1 block">Serviço</label>
                    <input value={formCourse} onChange={e => setFormCourse(e.target.value)} className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary" placeholder="Ex: Psicologia Hospitalar" />
                  </div>
                </>
              )}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="date" value={formDate} onChange={e => setFormDate(e.target.value)} className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary" />
                </div>
                <div>
                  <label className="text-xs font-medium text-gray-500 mb-1 block">Horário *</label>
                  <input type="time" value={formTime} onChange={e => setFormTime(e.target.value)} className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <button onClick={() => setFormType('voice_ai')} className={`flex-1 flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl text-sm font-medium border transition-colors ${formType === 'voice_ai' ? 'border-primary bg-primary/10 text-primary' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleSave} disabled={saving || !formPhone || !formDate || !formTime} className="flex-1 px-4 py-2.5 bg-primary text-white rounded-xl text-sm font-medium hover:bg-primary/90 disabled:opacity-50 transition-colors">
    ...
    <div className={`p-3.5 rounded-xl border ${s.status === 'cancelled' ? 'opacity-50' : ''} ${STATUS_MAP[s.status]?.bg || 'bg-gray-50 border-gray-200'}`}>
      <div className="flex items-start justify-between mb-2">
        <div className="flex items-center gap-2">
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${s.type === 'voice_ai' ? 'bg-primary/10' : 'bg-pink-50'}`}>
            {s.type === 'voice_ai' ? <Bot className="w-4 h-4 text-primary" /> : <UserCheck className="w-4 h-4 text-pink-500" />}
          </div>
          <div>
